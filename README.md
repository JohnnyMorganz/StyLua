...
```sh
aftman add johnnymorganz/stylua@0.13.1
```
...
Glob Filtering is only used for directory searching - passing a file directly (e.g. `stylua foo.txt`) will override the glob and format.
...
Note that `--verify` may produce false positives and negatives - we recommend manual checking as well as running tests to confirm.
